...
|------|------|
| **顶部导航** | 各模块快速入口 |
| **天气卡片** | 当前杭州天气实况（温度、体感、湿度、露点、干湿感、风速、天气现象） |
| **模块入口** | 四个功能模块的卡片导航 |
| **系统状态** | 各模块运行状态指示灯 |

### 2.2 快速导航

首页顶部天气卡片除了基础气温外，还会显示体感温度、露点和干湿感。其中露点和干湿感更适合辅助判断当前空气偏干还是偏湿、体感是否闷热。
...
在首页右下角点击 💬 浮动按钮，展开聊天窗口。

### 3.2 基本使用

直接输入自然语言问题，Agent 会自动调用系统各模块获取数据并回答。例如：
...
聊天记录会自动保存在浏览器中，刷新页面后 1 小时内可恢复。服务器重启后缓存自动清空。

### 3.5 图片查看

Agent 回复中的图表点击后可在当前页面悬浮放大查看，点击背景或按 ESC 关闭。
